...
class Application {
	...
	start() {
		switch( this.task ) {
			...
			default:
				...
				console.log( "No valid options provided. Try 'spodr help' to get a list of possible options." );
		}
	}
	...
	clean() {
		...
		return Promise.map( this.repositories, _.partial( tasks.clean, _, this.config ), {
			concurrency : this.config.parallelExecutions
		} )
			.then( () => this.finish() )
			.catch( err => {
	...
	update() {
		...
		// Skip updating the git repo if the user explicitly asked for it or if any
	...
	pmInstall( repositories ) {
		...
		if( 1 < this.config.parallelExecutions ) {
	...
	prepareRepositories() {
		...
		return Promise.map( repositories, repository => {
			...
			.filter( repository => !!repository.packageName );
	}
