...
class GitHubImporter {
	...
	process() {
		...
		return Promise.resolve( got( `https://api.github.com/orgs/${this.organization}/repos`, {
				...
				query   : {
					per_page : 100// jscs:ignore requireCamelCaseOrUpperCaseIdentifiers
				},
				json    : true
			} ) )
			.then( result => {
			...
			.map( project => {
				return {
					name    : project.name,// jscs:ignore requireCamelCaseOrUpperCaseIdentifiers
					url     : this.options.https ? project.clone_url : project.ssh_url,// jscs:ignore requireCamelCaseOrUpperCaseIdentifiers
					linkDep : true
				};
			} )
			.then( repositories => {
